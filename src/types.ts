...
export interface MoltbotEnv {
  ...
  OPENCLAW_VERBOSE?: string; // Set to 'true' to enable verbose gateway logging (causes memory growth)
  SANDBOX_SLEEP_AFTER?: string; // How long before sandbox sleeps: 'never' (default), or duration like '10m', '1h'
  TELEGRAM_BOT_TOKEN?: string;
  TELEGRAM_DM_POLICY?: string;
  TELEGRAM_DM_ALLOW_FROM?: string; // Comma-separated Telegram user IDs allowed to DM
  DISCORD_BOT_TOKEN?: string;
  DISCORD_DM_POLICY?: string;
  SLACK_BOT_TOKEN?: string;
  SLACK_APP_TOKEN?: string;
  GOOGLE_OAUTH_CREDENTIALS_JSON?: string; // Google OAuth client credentials JSON for gog skill
  GOOGLE_PLACES_API_KEY?: string; // Google Places API key for goplaces skill
